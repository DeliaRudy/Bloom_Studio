...
export function TravelMapClient() {
    ...
    const handleVisitedChange = (cities: City[]) => {
        ...
        cities.forEach(city => {
            if(!currentPinCities.includes(city.city)) {
                addDocumentNonBlocking(pinsCollection, {...cityToPin(city, 'visited'), sessionID: 'default'});
            }
        });
        // Remove old pins
    ...
    const handleWishlistChange = (cities: City[]) => {
        ...
        cities.forEach(city => {
            if(!currentPinCities.includes(city.city)) {
                addDocumentNonBlocking(pinsCollection, {...cityToPin(city, 'wishlist'), sessionID: 'default'});
            }
        });
        // Remove old pins
        pinsData?.forEach(pin => {
