...
const mockNotifications: Notification[] = [
  { id: '1', title: 'AI Reflection Ready!', description: 'Your weekly AI-powered reflection is now available to view.', read: false, date: '2 hours ago' },
  { id: '2', title: 'Goal Milestone Achieved', description: 'Congratulations! You completed "Launch my side business".', read: false, date: '1 day ago' },
  { id: '3', title: 'New "Rose Gold" Theme Unlocked', description: 'You can now select the new theme in your settings.', read: true, date: '3 days ago' },
  { id: '4', title: 'Cycle Phase Change', description: 'You are now entering your Luteal phase. Time to focus on details.', read: true, date: '5 days ago' },
];
...
export default function DemoNotificationsPage() {
  ...
  const handleSendTest = async () => {
    ...
    toast({
        ...
        description: `In a real app, an email would be sent to the user. This is a demo.`
    });
    setIsSending(false);
  }
  ...
  return (
    <div>
      <PageHeader title="Notifications & Settings" description="View recent updates and configure your email notification preferences." />
                ...
                filteredNotifications.map((notification, index) => (
                    ...
                    <div className="flex items-start justify-between gap-4 p-4 rounded-lg hover:bg-muted/50">
                        <div className="flex items-start gap-4">
                            <span className={cn("h-3 w-3 rounded-full mt-1.5 shrink-0", notification.read ? "bg-muted" : "bg-primary")} />
                            <div>
        ...
        <div className="space-y-8">
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Configure when you receive email reminders from Bloom.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                    <div className="flex items-center justify-between rounded-lg border p-4">
                        ...
                        <Switch id="enable-emails" checked={enableEmails} onCheckedChange={setEnableEmails} />
                    </div>
                    <div className={cn("space-y-4", !enableEmails && "opacity-50 pointer-events-none")}>
                        <div className="flex items-center justify-between">
                            ...
                            <Switch id="plan-day-reminder" checked={planDayReminder} onCheckedChange={setPlanDayReminder} />
                        </div>
                        <Separator/>
                        <div>
                            ...
                            <div className="grid grid-cols-3 gap-2">
                                {productivityReminders.map((time, index) => (
                                    <Input key={index} type="time" defaultValue={time} className="text-xs" />
                                ))}
                            </div>
                        </div>
                        <Separator/>
                        <div className="flex items-center justify-between">
                            ...
                            <Switch id="habits-reminder" checked={habitsReminder} onCheckedChange={setHabitsReminder} />
                        </div>
                        <Separator/>
                        <div className="flex items-center justify-between">
                            <Label htmlFor="reflection-reminder">End-of-Day Reflection Reminder (8pm)</Label>
                            <Switch id="reflection-reminder" checked={reflectionReminder} onCheckedChange={setReflectionReminder} />
                        </div>
                    </div>
                </CardContent>
                <CardFooter>
